...
# ------------------------------------------------------------------
# These symbols are consumed by vllm-ascend Worker / ModelRunner / patch
# code.  They live here (not in a patch file) because all callers are
...
def get_edge_cloud_local_indices(num_hidden_layers: int) -> set[int] | None:
...
def in_the_same_node_as_edge_cloud(
    ...
    """Edge-cloud-aware same-node detection for ``MessageQueue``.

    In edge-cloud mode the "same node" concept is replaced by "same side"
    (edge vs cloud).  All ranks on the edge side are considered "local" to
    each other, and all ranks on the cloud side are "local" to each other,
    regardless of physical node boundaries.
    """
