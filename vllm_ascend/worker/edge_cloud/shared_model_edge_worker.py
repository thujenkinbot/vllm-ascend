...
class _BatchedExecuteMarker(DeferredExecutePostprocess):
    ...
    * :meth:`drain_batched_round` (class function) — Phase B/C: collect
    ...
    def drive_batched_round(
        ...
        self.worker._pp_send_work = get_pp_group().isend_tensor_dict(
            hidden_k.tensors, dst=dp_rank + 1)
        pending_deferred[dp_rank] = (
            self.worker.make_batched_recv_closure(src=dp_rank + 1))
...
class SharedModelEdgeWorker(NPUWorker):
    ...
    def make_batched_recv_closure(self, src: int):
        ...
        The closure stores the received ``comm_postprocess`` handles
        on the returned ``AsyncIntermediateTensors``; the batched
        tail call in the busy_loop awaits them via
        ``AsyncIntermediateTensors`` (mirrors the original
        ``DeferredExecutePostprocess`` semantics in
        ``SharedModelEdgeWorker.execute_model``).
        """
        def _recv():
            from vllm_ascend.distributed.parallel_state import (
                edge_cloud_broadcast_recv,
            ...
            tensor_dict, comm_handles, comm_postprocess = (
                edge_cloud_broadcast_recv(src=src))
